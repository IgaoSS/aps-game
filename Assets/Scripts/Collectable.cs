...
public class Collectable : MonoBehaviourPunCallbacks
{
    ...
    void Start()
    {
        transform.Rotate(new Vector3(0, 0, Random.Range(-120, -60)));
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player"))
        {
            ...
            if (photonView && photonView.IsMine)
            {
                ...
                GameController.Instance.photonView.RPC("AddScore", RpcTarget.AllBuffered, photonView.ViewID, pointValue);
            }
            Destroy(gameObject);
        }
    }
